...
const Register = () => {
    ...
    const joinGroupId = location.state?.groupId;
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const user = await register(formData.name, formData.email, formData.password, 'single');
            if (user) {
    ...
    const handleStepperFinish = async () => {
        if (shouldJoinGroup && joinGroupId) {
            try {
                await groupAPI.join(joinGroupId);
                navigate('/quiz');
            } catch (err) {
                console.error('Failed to join group after registration:', err);
