...
function createProgram(gl, shaderSources, transformFeedbackVaryings = null, attribLocations = null) {
...
function makeVertexArray(gl, bufLocNumElmPairs, indices = null) {
...
class ArcballControl {
  constructor(canvas, updateCallback = () => undefined) {
    this.canvas = canvas;
...
class InfiniteGridMenu {
  ...
  initTexture() {
    ...
    Promise.all(this.items.map((item) => new Promise((resolve) => {
      ...
      img.onerror = () => { console.warn(`Failed to load ${item.image}`); resolve(null); };
      img.src = item.image;
    }))).then((images) => {
...
const InfiniteMenu = ({ items = [], scale = 1.0 }) => {
  ...
  return (
    <div className="relative w-full h-[600px] overflow-hidden rounded-2xl" style={{ backgroundColor: '#1A0F0A' }}>
      {isLoading ? (
        ...
        <>
          ...
          {activeItem && (
            <>
              <h2
                className={`select-none absolute font-black [font-size:3rem] left-[1.6em] top-1/2 transform translate-x-[20%] -translate-y-1/2 transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${
              ...
              <p
                className={`select-none absolute max-w-[15ch] text-[1.2rem] top-1/2 right-[1%] transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${
              ...
              <div
                ...
                className={`absolute left-1/2 z-10 w-[60px] h-[60px] grid place-items-center rounded-full cursor-pointer transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${
                ...
                style={{ backgroundColor: '#8B5A2B', border: '5px solid #1A0F0A' }}
              >
                <span className="select-none relative text-white top-[2px] text-[26px]">&#x2197;</span>
              </div>
            </>
          )}
        </>
